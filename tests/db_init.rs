...
fn test_database_initialization() {
    ...
    let mut stmt = conn.prepare("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='journal_entries'").unwrap();
    ...
    conn.execute("INSERT INTO journal_entries (entry, tags) VALUES ('Test entry', 'test')", [])
        .expect("Failed to insert test data");

    // Verify the data was inserted
    let mut stmt = conn.prepare("SELECT COUNT(*) FROM journal_entries").unwrap();
    let count: i64 = stmt.query_row([], |row| row.get(0)).unwrap();
